...
function renderCourseGrid(courses) {
    ...
    courses.forEach(course => {
        ...
        `;
        // Make whole card clickable usually, but we have specific buttons now.
...
async function loadCourseHierarchy(courseId) {
    courseTreeRoot.innerHTML = '<div style="padding:1rem; color:#888;">Loading structure...</div>';
    try {
        // Fetch Course Info for Header
        // We don't have getCourseById API yet exposed clearly in api.js, assume we have full list or fetch again.
        // Let's use getCourses() and find for now, or assume we passed it.
        // Correct way: Add getCourseById API. For now, lazy find.
        const courses = await api.getCourses();
...
function renderSidebarTree(chapters) {
    ...
    if (chapters.length === 0) {
        courseTreeRoot.innerHTML = '<div style="padding:1rem; font-style:italic; color:#666">No chapters yet. Add one above.</div>';
        return;
    }
...
function createSidebarNode(item, type) {
    ...
    // Contextual Actions (Add Lesson, Delete)
    // We can add these on hover or right click. Let's make it simple: buttons inline or generic select.
    // User requested: "Sidebar allows managing Chapters and Lessons"
    ...
    // Expand Logic
    if (hasChildren) {
        // Toggle on click header for chapters
        nodeHeader.onclick = async (e) => {
            // If clicking actions, stop
        ...
        // Auto-expand if state persisted
        if (isExpanded) {
            loadLessons(item.chapter_id, childrenContainer);
        }

    } else {
        // Lesson Click -> Load Content Editor
        nodeHeader.onclick = () => {
...
async function loadLessons(chapterId, container) {
    container.innerHTML = '<div style="padding-left:1.5rem; font-size:0.8rem; color:#666;">Loading...</div>';
    try {
        const lessons = await api.getLessons(chapterId);
        container.innerHTML = '';
        if (lessons.length === 0) {
            container.innerHTML = '<div style="padding-left:1.5rem; font-size:0.8rem; color:#666">No basic lessons</div>';
            return;
        }
...
window.loadChapterEditor = async (chapter) => {
    editorArea.innerHTML = '<div style="padding:2rem;">Đang tải nội dung...</div>';

    try {
        let exercises = [];
        try { exercises = await api.getExercisesByChapter(chapter.chapter_id); } catch (e) { }
        ...
        editorArea.innerHTML = '<div style="padding:2rem; color:red">Lỗi tải nội dung</div>';
    }
};

// --- Content Editor (Right Side) ---

// Global cache for current lesson contents to avoid re-fetching on small reorders
let currentLessonContentsMap = [];

window.moveContent = async (contentId, direction, lessonId, btn) => {
    // Find the current list in DOM or from cache? Cache is better but we need to keep it synced.
    // Let's rely on the DOM order or re-fetch.
    // Actually, simplest is to re-use the 'contents' array if we had it globally.
    // Since we don't, let's grab the current order from api.getContents first (safest) OR
    // just swap in the UI and send the list.
...
function renderLessonEditor(lesson, contents, exercises = []) {
    editorArea.innerHTML = `
        ...
        <div class="child-list">
            ...
            <div id="content-list">
                ${contents.map(c => {
        const safeEnc = (str) => encodeURIComponent(str || '').replace(/'/g, "%27");
        return `
...
function showModal(title, contentHTML, verifyAction) {
    ...
    // Replace save button to clear listeners
    const newSave = modalSave.cloneNode(true);
    modalSave.parentNode.replaceChild(newSave, modalSave);
    modalSave = newSave; // Update ref is safe here as we use querySelector if needed, but var update works mostly.
...
window.createChild = async (type, parentId) => {
    if (type === 'chapter') {
        // Use the global currentCourseId if parentId is missing or invalid context (e.g. from generic onclick)
        const courseIdToUse = currentCourseId;

        showModal('New Chapter',
            ...
            async () => {
                ...
                if (!name) throw new Error("Name required");
                await api.createChapter({ course_id: courseIdToUse, chapter_name: name });
                loadCourseHierarchy(courseIdToUse);
            }
        );
    } else if (type === 'lesson') {
        showModal('New Lesson',
...
window.createContent = async (lessonId) => {
    ...
    showModal('Add Content', html, async () => {
        ...
        await api.createContent({
            ...
            content_type: type,
            content_title: title,
            // content_order: 1, // REMOVED: Let backend handle auto-increment
            body: body || '',
    ...
    // Helper to toggle fields immediately after render
...
window.createExercise = async (id, parentType, isChapter = false) => {
    // id param is lessonId by default, unless isChapter=true then it is chapterId
    ...
    showModal('Thêm Bài Tập Mới', html, async () => {
        ...
        if (isChapter) {
            loadChapterEditor({ chapter_id: chapterId, chapter_name: "Đang tải lại..." });
        } else {
...
window.editExercise = async (id, encodedTitle, parentId, isChapter = false) => {
    try {
        ...
        showModal('Sửa Bài Tập', html, async () => {
            ...
            if (modalContent) modalContent.classList.remove('modal-wide');
            if (isChapter) {
                // If chapter editor, trying to refresh is a bit generic due to lack of ID in args, 
                // but usually loadCourseHierarchy re-renders mostly. 
                // Better: reloadChapterEditor if we had the ID. 
                // Since we don't strictly pass chapterId here, we can reuse the parentId logic if it was passed correctly?
                // Actually parentId IS the lessonId OR null if chapter.
                // If isChapter is true, parentId (which is lessonId in args) is likely null?
                // Looking at caller: editExercise(..., null, true) -> parentId is null.
                // We need to reload just the sidebar or the content.
                // Let's just alert for now or try to reload dashboard if confused.
                // Actually, the caller `editExercise` logic in render has: 
                // createExercise passes chapter_id as first arg if isChapter=true.
                // But editExercise in render passes: editExercise(id, title, null, true).
                // So we don't have chapterId easily to reload.
...
window.manageTestCases = async (exerciseId) => {
    ...
    `;
        ...
        showModal('Quản lý Test Cases', html, async () => {
            // Main Save button just closes
            return true;
        });

        // Bind internal Add button
        setTimeout(() => {
            ...
            if (addBtn) {
                addBtn.onclick = async (e) => {
                    ...
                    if (!output) { alert("Output required"); return; }
                    ...
                    showTestCasesModal(); // Reload modal content
                };
            }
        }, 100);
    };
...
// Global for inline calls
window.deleteTestCase = async (id, exerciseId) => {
    if (!confirm("Delete case?")) return;
    await api.deleteTestCase(id);
    // Re-trigger modal open to refresh (a bit hacky but works for now without full state man)
...
// Safe Helpers to avoid string escaping in HTML
...
window.editContent = (id, type, encodedTitle, encodedBody, lessonId, encodedExpl, encodedLang) => {
    // Decode fields
    const title = decodeURIComponent(encodedTitle);
    const body = decodeURIComponent(encodedBody);
    const expl = encodedExpl && encodedExpl !== 'undefined' ? decodeURIComponent(encodedExpl) : '';
    const currentLang = encodedLang && encodedLang !== 'undefined' ? decodeURIComponent(encodedLang) : 'javascript';
    ...
    showModal('Edit Content', html, async () => {
        ...
        const newExpl = type === 'code' ? document.getElementById('edit-cont-expl').value : null;
        const newLang = type === 'code' ? document.getElementById('edit-cont-lang').value : null;
...
window.deleteItem = async (type, id, parentId) => {
    ...
    if (type === 'course') {
        ...
        const isChapter = parentId === null || parentId === undefined; // HACK: detecting if we passed lessonID (number) or nothing/null
        // Wait, deleteItem params were (type, id, parentId). 
        // In chapter editor we called: deleteItem('exercise', id, null, true)
        // We need to support 4th arg in deleteItem signature if we want explicit flag, 
        // OR reuse parentId as optional.

        await api.deleteExercise(id);
        // Refresh: If lesson exercise, we have parentId (lessonId). If chapter, parentId is null.
        if (parentId) {
            loadLessonEditor({ lesson_id: parentId, lesson_name: "Đang tải lại..." });
        } else {
...
// Edit Meta Placeholders (to be fully implemented if requested)
window.editCourseMeta = (id) => alert("Edit Course Meta: To be implemented fully separate from Dashboard");
window.editItem = (type, id, encodedName) => {
    const currentName = encodedName && encodedName !== 'undefined' ? decodeURIComponent(encodedName) : '';
    ...
    if (type === 'chapter') {
        ...
        showModal('Sửa Tên Chương', html, async () => {
            ...
            // Reload sidebar and editor
            loadCourseHierarchy(currentCourseId);
            // If we are currently viewing this chapter, reload the header title too by reloading the editor
            // But loadCourseHierarchy might not update the main view content. 
            // Let's reload the chapter editor specifically to update the title there.
            loadChapterEditor({ chapter_id: id, chapter_name: newName });
        });
    } else if (type === 'lesson') {
        // ... existing lesson logic or placeholder if needed, for now just basic prompt or improve later
        // The user specifically asked for Chapter edit in this task.
        const newName = prompt("Enter new name:", currentName);
        if (newName) {
            fetch(`/api/lessons/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lesson_name: newName }) })
